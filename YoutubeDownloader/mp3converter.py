...
for file in os.listdir(r"C:\_work\Musik\schniiidi"):
    webmFile = os.path.join(r"C:\_work\Musik\schniiidi", file)
    mp3File = os.path.join(r"C:\_work\Musik\schniiidi", file).replace(
        "webm", "mp3")

    ffmpeg_path = "C:\\ffmpeg-2024-04-15-git-5e380bcdb1-full_build\\bin\\ffmpeg.exe"
    command = f"{ffmpeg_path} -i \"{webmFile}\" -vn -ab 128k -ar 44100 -y \"{mp3File}\""
